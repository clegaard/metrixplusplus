...
class Plugin(mpp.api.Plugin, mpp.api.Parent, mpp.api.IConfigurable, mpp.api.IRunable):
    ...
    def configure(self, options):
        ...
        try:
            self.add_include_rule(re.compile(
                options.__dict__['include_files']))
        except Exception as e:
            self.optparser.error("option --include-files: " + str(e))
        try:
            self.add_exclude_rule(re.compile(
                options.__dict__['exclude_files']))
        except Exception as e:
    ...
    def initialize(self):
        ...
        super(Plugin, self).initialize(namespace='std.general',
                                       support_regions=False, fields=fields)
        self.add_exclude_file(self.get_plugin('mpp.dbf').get_dbfile_path())
        self.add_exclude_file(self.get_plugin(
            'mpp.dbf').get_dbfile_prev_path())
...
class DirectoryReader():
    ...
    def run(self, plugin, directory):
                ...
                if os.path.isdir(full_path):
                    ...
                    if parser == None:
                        ...
                        if plugin.is_procerrors_enabled == True and procerrors != None and procerrors != 0:
                            data.set_data(
                                'std.general', 'procerrors', procerrors)
        ...
        if os.path.isdir(directory):
            ...
            total_errors = run_per_file(
                plugin, os.path.basename(directory), directory)
