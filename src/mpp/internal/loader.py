...
class MultiOptionParser(optparse.OptionParser):
    ...
    def __init__(self, *args, **kwargs):
        optparse.OptionParser.__init__(
            self, *args, option_class=self.MultipleOption, **kwargs)
...
class Loader(object):
    ...
    def load(self, command, directories, args):
                        ...
                        for (ind, action) in enumerate(item['actions']):
                            ...
                            if action == '*' or action == command:
                                active_plugins.append(
                                    item['package'] + '.' + item['module'])
                        inicontainer.plugins.append(item)
                        inicontainer.hash[item['package'] +
                                          '.' + item['module']] = item
            return active_plugins
        ...
        std_ext_dir = os.path.join(
            os.environ['METRIXPLUSPLUS_INSTALL_DIR'], 'ext')
        std_ext_priority_dir = os.path.dirname(
        ...
        for plugin_name in required_and_dependant_plugins:
            ...
            imp = f"{item['package']}.{item['module']}"
            #plugin = import_module(item['package'], item['module'])
            plugin = import_module(imp)
            instance = getattr(plugin, item['class'])()
            instance.set_name(item['package'] + "." + item['module'])
            instance.set_version(item['version'])
            instance._set_plugin_loader(self)
            item["instance"] = instance
            # module_attr = plugin.__getattribute__(item['class'])
        ...
        if command not in inicontainer.actions:
            optparser.error(
                "action {action}: Unknown command".format(action=command))
