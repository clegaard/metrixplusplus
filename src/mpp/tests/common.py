...
class ToolRunner(object):
    ...
    def __init__(self,
        ...
        self.test_location = os.path.join(
            self.suite_location, test_name + "_" + tool_name + "_" + str(prefix))

        db_file = os.path.join(
            os.environ['METRIXPLUSPLUS_INSTALL_DIR'], self.suite_location, test_name + ".db")
        self.dbfile = db_file
        if (remove_exiting_dbfile == True or (remove_exiting_dbfile == None and tool_name == 'collect')) and os.path.exists(db_file):
            os.unlink(db_file)

        db_file_prev = os.path.join(
            os.environ['METRIXPLUSPLUS_INSTALL_DIR'], self.suite_location, test_name + ".prev.db")
    ...
    def run(self):
        ...
        # Match with gold
        self.is_stdout_matched = self.inetrnal_compare_with_gold(
            child_stdout, gold_file_stdout, real_file_stdout, diff_file_stdout)
        if self.stderr_lines != None:
            self.is_stderr_matched = self.inetrnal_compare_with_gold(
                child_stderr, gold_file_stderr, real_file_stderr, diff_file_stderr, self.stderr_lines)
        else:
    ...
    def inetrnal_compare_with_gold(self, text, gold_file, real_file, diff_file, lines=None):
        ...
        if result == False:
            ...
            diff_text = difflib.HtmlDiff().make_file(gold_to_compare.splitlines(),
                                                     text_to_compare.splitlines(), "Gold file", "Real output")
            f = open(diff_file, 'w')
            f.write(diff_text)
            f.close()
        else:
    ...
    def check_all(self):
        ...
        if result == False:
            self.message += "\nCheck for exit code: " + str(self.check_exit_code()) \
                ...
                "\nCheck for stdout: " + \
                str(self.check_stdout()) + "\nCheck for stderr: " + \
                str(self.check_stderr())
        return result
...
class TestCase(unittest.TestCase):
    ...
    def get_content_paths(self, cwd='sources'):
        ...
        return ContentPaths(os.path.join('tests', group_name, suite_name, cwd),
                            os.path.join(os.environ['METRIXPLUSPLUS_INSTALL_DIR'],
                                         'tests', group_name, suite_name, test_name + ".db"),
    ...
    def setUp(self):
        ...
        logging.basicConfig(
            format="[TEST-LOG]: %(levelname)s:\t%(message)s", level=logging.WARN)
